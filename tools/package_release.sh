...
#   devday-terminal-update-VERSION.signed.bin    signed app-only OTA image
...
OUT="$ROOT/release"
KEY="${SIGNING_KEY:-$ROOT/tools/keys/update_key.pem}"
...
UPDATE="$OUT/devday-terminal-update-$VERSION.signed.bin"
RECOVERY="$OUT/devday-terminal-recovery-$VERSION.bin"

cp "$MERGED" "$FACTORY"
cp "$APP" "$RECOVERY"
"$ROOT/tools/sign_update.sh" "$APP" "$UPDATE" "$KEY"

GIT_REV="$(git -C "$ROOT" rev-parse --short HEAD 2>/dev/null || echo unknown)"
BUILD_DATE="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

( cd "$OUT" && shasum -a 256 "devday-terminal-factory-$VERSION.bin" \
  "devday-terminal-update-$VERSION.signed.bin" \
